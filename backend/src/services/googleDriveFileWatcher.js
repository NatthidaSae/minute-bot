...
class GoogleDriveFileWatcher {
  ...
  async processGoogleDriveFile(fileMetadata) {
    try {
      ...
      // Check if file already processed by filename //problem 01
      ...
      if (!isRetry) {
        ...
        const { meetingName, transcriptTitle, meetingDate, meetingTime } = extractMeetingInfoFromFilename(filename);
        const effectiveDate = meetingDate || new Date();

        // Create or find meeting record
        const meetingResult = await this.findOrCreateMeeting(meetingName, effectiveDate, meetingTime);
   ...
   // Find or create a meeting record // problem 02
   
  async findOrCreateMeeting(meetingName, meetingDate, meetingTime = null) {
    ...
    // Check if meeting exists with the same title, date, and time
    // This ensures we don't create duplicate meetings for the same time slot
    const existingMeeting = await pool.query(
      `SELECT id, title FROM meetings 
       WHERE LOWER(TRIM(title)) = LOWER(TRIM($1)) 
       AND user_id = $2
       AND meeting_date = $3
       AND (meeting_time = $4 OR (meeting_time IS NULL AND $4 IS NULL))
       LIMIT 1`,
      [meetingName, userId, meetingDate, meetingTime]
    );
    ...
    // Include both date and time for the meeting with Asia/Bangkok timezone
    const newMeeting = await pool.query(
      'INSERT INTO meetings (title, meeting_date, meeting_time, timezone, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING id, title',
      [meetingName, meetingDate, meetingTime, 'Asia/Bangkok', userId]
    );
